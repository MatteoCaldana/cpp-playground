...
std::vector<Particle<T, dim>> initialize(size_t n) {
  ...
  for(size_t i = 0; i < n; ++i){
    particles.push_back({1.0, Vec::Random(), Vec::Zero(), Vec::Zero()});
...
template<typename T, int dim>
void plot(Gnuplot &gp, const std::vector<Particle<T, dim>> &particles) {
...

  {
    ...
    for(double t = 0.0; t < t_fin; t += dt) {
      ...
      tree.traverse_bfs([&](const auto &node){
        gp << "plot '-' with lines\n";
        std::vector<double> x = {node.corners(0, 0), node.corners(0, 1), node.corners(0, 1), node.corners(0, 0), node.corners(0, 0)};
        std::vector<double> y = {node.corners(1, 0), node.corners(1, 0), node.corners(1, 1), node.corners(1, 1), node.corners(1, 0),};
        gp.send1d(std::make_tuple(x, y));
        if(node.children_idx == -1)
          return true;
        return false;
